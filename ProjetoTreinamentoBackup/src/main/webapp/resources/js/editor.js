/*
 * Criado pelo Guilherme 22/08/2014 para traduzir os botões do <p:editor> que por padrão são em Inglês
 */

(function(C) {
    C.cleditor = {
        defaultOptions: {width: 500, height: 250, controls: "bold italic underline strikethrough subscript superscript | font size style | color highlight removeformat | bullets numbering | outdent indent | alignleft center alignright justify | undo redo | rule image link unlink | cut copy paste pastetext | print source", colors: "FFF FCC FC9 FF9 FFC 9F9 9FF CFF CCF FCF CCC F66 F96 FF6 FF3 6F9 3FF 6FF 99F F9F BBB F00 F90 FC6 FF0 3F3 6CC 3CF 66C C6C 999 C00 F60 FC3 FC0 3C0 0CC 36F 63F C3C 666 900 C60 C93 990 090 399 33F 60C 939 333 600 930 963 660 060 366 009 339 636 000 300 630 633 330 030 033 006 309 303", fonts: "Arial,Arial Black,Comic Sans MS,Courier New,Narrow,Garamond,Georgia,Impact,Sans Serif,Serif,Tahoma,Trebuchet MS,Verdana", sizes: "1,2,3,4,5,6,7", styles: [
                ["Paragraph", "<p>"], ["Header 1", "<h1>"
                ], ["Header 2", "<h2>"], ["Header 3", "<h3>"], ["Header 4",
                    "<h4>"], ["Header 5", "<h5>"], ["Header 6", "<h6>"]
            ], useCSS: false, docType: '<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">', docCSSFile: "", bodyStyle: "margin:4px; font:10pt Arial,Verdana; cursor:text"}, 
        buttons: {init: "bold,Negrito,|italic,Italico,|underline,Sublinhado,|strikethrough,,|subscript,,|superscript,,|font,,fontname,|size,Tamanho Font,fontsize,|style,,formatblock,|color,Cor da Font,forecolor,|highlight,Cor de Fundo,hilitecolor,color|removeformat,Remove Formatting,|bullets,,insertunorderedlist|numbering,,insertorderedlist|outdent,,|indent,,|alignleft,Alinhar texto a esquerda,justifyleft|center,Centralizar Texto,justifycenter|alignright,Alinhar texto a direita,justifyright|justify,Texto Justificado,justifyfull|undo,Desfazer,|redo,Refazer,|rule,Inserir linha Horizontal,inserthorizontalrule|image,Inserir Imagem,insertimage,url|link,Insert Hyperlink,createlink,url|unlink,Remove Hyperlink,|cut,Recortar,|copy,Copiar,|paste,Colar,|pastetext,Colar o Texto,inserthtml,|print,Imprimir,|source,Codigo Fonte"}, imagesPath: function() {
            return t()
        }};
    ...
    var n = "backgroundColor", Z = "button", Y = "buttonName", A = "change", d = "cleditor", Q = "click", z = "disabled", D = "<div>", w = "transparent", M = "unselectable", b = "ui-editor", u = "ui-editor-toolbar", aa = "ui-editor-group", L = "ui-editor-button", ab = "ui-editor-disabled", J = "ui-editor-divider", G = "ui-editor-popup", ad = "ui-editor-list", U = "ui-editor-color", c = "ui-editor-prompt", W = "ui-editor-message", H = C.browser.msie, q = /msie\s6/i.test(navigator.userAgent), l = /iphone|ipad|ipod/i.test(navigator.userAgent), g = {}, B, v = C.cleditor.buttons;
    C.each(v.init.split("|"), function(af, ai) {
        var ag = ai.split(","), ah = ag[0];
        v[ah] = {stripIndex: af, name: ah, title: ag[1] === "" ? ah.charAt(0).toUpperCase() + ah.substr(1) : ag[1], command: ag[2] === "" ? ah : ag[2], popupName: ag[3] === "" ? ah : ag[3]}
    });
    delete v.init;
    ...
    function e(ak) {
        ...
        var ah = {editor: aj, button: am, buttonName: an, popup: ag, popupName: al, command: ai.command, useCSS: aj.options.useCSS};
    ...
    function T(al) {
        ...
        var ah = {editor: aj, button: ao, buttonName: ap, popup: af, popupName: ai.popupName, command: ag, value: an, useCSS: ak};
    ...
    function ac(ah) {
        var aj = ah.$main, ag = ah.options;
        if (ah.$frame) {
            ah.$frame.remove()
        }
        var ai = ah.$frame = C('<iframe frameborder="0" src="javascript:true;">').hide().appendTo(aj);
        var af = ai[0].contentWindow, ak = ah.doc = af.document, al = C(ak);
        ak.open();
        ak.write(ag.docType + "<html>" + ((ag.docCSSFile === "") ? "" : '<head><link rel="stylesheet" type="text/css" href="' + ag.docCSSFile + '" /></head>') + '<body style="' + ag.bodyStyle + '"></body></html>');
        ak.close();
        if (H) {
            al.click(function() {
                R(ah)
        ...
        N(ah);
        if (H) {
            al.bind("beforedeactivate beforeactivate selectionchange keypress", function(am) {
                if (am.type == "beforedeactivate") {
                    ah.inactive = true
                } else {
                    if (am.type == "beforeactivate") {
                        if (!ah.inactive && ah.range && ah.range.length > 1) {
                            ah.range.shift()
                        }
                        delete ah.inactive
                    } else {
                        if (!ah.inactive) {
                            ...
                            ah.range.unshift(j(ah));
                            while (ah.range.length > 2) {
                                ah.range.pop()
                            }
                        }
                    }
        ...
        (C.browser.mozilla ? al : C(af)).blur(function() {
            K(ah, true)
        });
        al.click(ae).bind("keyup mouseup", function() {
            o(ah)
        });
        if (l) {
            ah.$area.show()
        ...
        C(function() {
            var an = ah.$toolbar, ao = an.children("div:last"), ap = aj.width();
            var am = ao.offset().top + ao.outerHeight() - an.offset().top + 1;
            an.height(am);
            am = (/%/.test("" + ag.height) ? aj.height() : parseInt(ag.height)) - am;
            ai.width(ap).height(am);
            ah.$area.width(ap).height(q ? am - 2 : am);
            i(ah, ah.disabled);
            o(ah)
        })
    }
    function o(af) {
        ...
        C.each(af.$toolbar.find("." + L), function(ai, an) {
            ...
            if (af.disabled) {
                ...
                if (al.getEnabled) {
                    var ao = {editor: af, button: an, buttonName: al.name, popup: g[al.popupName], popupName: al.popupName, command: al.command, useCSS: af.options.useCSS};
                    ak = al.getEnabled(ao);
                    if (ak === undefined) {
                        ak = true
                    }
...
PrimeFaces.widget.Editor = function(b, a) {
    this.id = b;
    this.cfg = a;
    this.jq = jQuery(PrimeFaces.escapeClientId(b));
    if (!this.cfg.lazy) {
        this.init()
    }
};
