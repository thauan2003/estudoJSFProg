...
$(document).ready(function () {
    ...
    /*Guilherme - selectOneMenu - begin*/
    loopCombos();
    addIconAutoComplete();
    addStyleTooltip();

    $(".md-onemenu").find("select").on("change", function () {
        var idCombo = this.id;
        idCombo = idCombo.replace(/:/gi, '\\:');
        var descricao = $('#' + idCombo + ' option:selected').text();
        var valueCombo = $(this).val();
        var elemnto = $(this).closest(".md-onemenu");
        var label = elemnto.find('.lblCmb');
        if (descricao !== "" && valueCombo !== "undefined") {
            label.removeClass("lblCmb2").removeClass("lblCmb");
            label.addClass("lblCmb2");
            label.css("top", "-10px");
        } else if (descricao === "" && valueCombo !== "undefined") {
            if (label.length)
            {
                label.removeClass("lblCmb");
            } else {
                label = elemnto.find('.lblCmb2');
                label.removeClass("lblCmb2");
            }
            label.addClass("lblCmb");
            label.css("top", "10px");
        }
    });

    $('.md-onemenu>.ui-selectonemenu').click(function () {
        var elemnto = $(this).closest(".md-onemenu");
        var label = elemnto.find('.lblCmb');
        if (label.length)
        {
            label.removeClass("lblCmb").addClass("lblCmb2");
        } else {
            label = elemnto.find('.lblCmb2');
            label.removeClass("lblCmb2").addClass("lblCmb");
        }
        label.css("top", "-10px");
    });
    /*Guilherme - selectOneMenu - end*/

    /*Guilherme- 05-07-2017 jquery para criação do menu com botões. (Controle Profissional) - Begin*/
    var menuButton = $('.menuBotoes').find(".ui-menubutton");
    menuButton.each(function () {
        hideShowDivSubMenu(this.id);
        $(this).find(".ui-button-text").after("<span class='ui-button-icon-right ui-icon ui-icon-triangle-1-e'></span>");
        $(this).mouseenter(function () {
            var idMenuButton = this.id;
            idMenuButton = idMenuButton.replace(/:/gi, '\\:');
            var offset = $(this).position();
            $('#' + idMenuButton + '_menu').css({
            ...
            var idMenuButton = this.id;
            idMenuButton = idMenuButton.replace(/:/gi, '\\:');
            $('#' + idMenuButton + '_menu').hide();
        });
    });
    /*end - menu com botões*/
});

function hideShowDivSubMenu(idComponent) {
    idComponent = idComponent.replace(/:/gi, '\\:');
    $("#" + idComponent + "_menu").mouseenter(function () {
        var offset = $("#" + idComponent).position();
        $('#' + idComponent + '_menu').css({
            top: (offset.top + 30) + 'px',
            left: (offset.left) + 'px'
        }).show();
        $('#' + idComponent + '_button').addClass("ui-state-hover");
    }).mouseleave(function () {
        $('#' + idComponent + '_menu').hide();
        $('#' + idComponent + '_button').removeClass("ui-state-hover");
    });
}

function addStyleCombo() {
    try {
        loopCombos();
...
function loopCombos() {
    $('.md-onemenu select').each(function () {
        var idCombo = this.id;
        idCombo = idCombo.replace(/:/gi, '\\:');
        var descricao = $('#' + idCombo + ' option:selected').text();
...
function addIconAutoComplete() {
    $('.autocompleteIcon').each(function () {
        var inputgroup = $(this).parent(".ui-inputgroup");
        if (!inputgroup.length) {
            $(this).wrap("<div class=\"ui-inputgroup\"></div>");
            $(this).before("<span class=\"ui-inputgroup-addon\"><i class=\"fa fa-search\" style=\"color: #b5b5b5\"></i></span>");
        }
    });
}

function addClassColorTheme(idComponent) {
    if (!$(idComponent).hasClass("background-colorTheme")) {
        $(idComponent).addClass("background-colorTheme");
    }
}

function aplyRequired() {
    var myArray = document.querySelectorAll("[aria-required='true']");
    $.each(myArray, function () {
        var tdDentro = $(this).closest(".tdStyleGrad");
        if (tdDentro.length) {
            tdDentro.find(".lblCssTd").each(function () {
                if ($(this).length) {
                    var temReq = false;
                    $(this).find(".reqRed").each(function () {
                        temReq = true;
                    });
                    if (!temReq) {
                        $(this).append("<span class='reqRed' style='color: red'> *</span>");
                    }
                }
            });
        }
...
function addStyleTooltip() {
    $(".ui-tooltip").each(function () {
        var tooltipText = $(this).find(".ui-tooltip-text");
        if (tooltipText.length)
        {
            tooltipText.removeClass("background-colorTheme").addClass("background-colorTheme");
            tooltipText.fadeTo(0, 1);
        }
    });
}


function valorTecla(e) {
    var tecla = e.keyCode ? e.keyCode : e.which ? e.which : e.charCode;
    return tecla;
}

function criaTabEnter(idForm) {
    $('#'+idForm).on('keydown', 'input, textarea', function (event) {
        var tecla = valorTecla(event);
        if (tecla === 13 || tecla === 9) {
            var $this = $(event.target);
            var tipo = $this.attr('type');
            var index = parseFloat($this.attr('tabindex'));
            var obj = $('[tabindex="' + (index + 1).toString() + '"]');
            if (tipo !== 'submit') {
                event.preventDefault();
            }
            if (obj.length && obj.is(':visible') && !obj.is(':disabled')) {
                if (obj.attr('type') === 'submit') {
                    obj.focus();
                } else if (obj.attr('type') === 'radio') {
                    obj.on('focus', function () {
                        var $ui_radiobutton = obj.closest('.ui-selectoneradio');
                        $ui_radiobutton.find('.ui-radiobutton-box').addClass('focusRadio');
                    });
                    obj.on('focusout', function () {
                        var $ui_radiobutton = obj.closest('.ui-selectoneradio');
                        $ui_radiobutton.find('.ui-radiobutton-box').removeClass('focusRadio');
                    });
                    obj.focus();
                } else {
                    obj.select();
